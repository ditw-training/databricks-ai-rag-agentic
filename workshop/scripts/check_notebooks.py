...
FORBIDDEN = [
    ...
    (r"from langchain_classic", "agenci stoją na langchain.agents.create_agent (LangChain 1.x)"),
    (r"AgentExecutor\(", "AgentExecutor jest w langchain_classic; nowe agenty buduje create_agent"),
    ...
    (r"\d+\s?(min\b|minut)|\*\*Czas:\*\*", "no minutes in notebooks: timing lives in documents/scripts/schedule.md"),
    ...
    (r"^\s*global\s+\w", "globalna zmienna w notebooku: drugie uruchomienie komórki daje inny wynik"),
    (r"while True:", "własna pętla pollingu; SDK ma wait_until_ready()/wait_for_endpoint(verbose=True)"),
    (r"^\s*(?:#|--|<!--)\s*source:", "pochodzenie komórki trzymamy w metadata.source_ref, a nie w treści, którą widzi uczestnik"),
]
# Pochodzenie komórki (WSx[i], slide N, PRZ[i], K:ścieżka, new) leży w metadanych, żeby nie było go widać w notebooku.
MARKER_KEY = "source_ref"
...
def marker_of(cell: dict) -> str | None:
    value = cell.get("metadata", {}).get(MARKER_KEY)
    ...
    for index, cell in enumerate(nb.get("cells", []), 1):
        ...
        if cell["cell_type"] == "code":
            ...
            if require_markers and not stripped.startswith("%pip") and marker_of(cell) is None:
                findings.append(f"{where}: missing `metadata.{MARKER_KEY}`")
